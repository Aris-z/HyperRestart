...
from transformers.models.t5.modeling_t5 import (T5PreTrainedModel, T5LayerNorm, T5Block,
                                      T5Attention, T5LayerCrossAttention)


from hyperspace.HyperAdapter.adapter import AdapterLayer
from hyperspace.HyperAdapter.hyperAdapter import HyperLayerAdapter, HyperBlockAdapter, HyperBlockAdapterUnique, HyperLayerAdapterUnique
from hyperspace.HyperAdapter.hyperTask import TaskEmbedHyperNet
from hyperspace.HyperAdapter.hyperLayerNorm import LayerNormHyperNet
...
with open("hsgdr/configs/meta_adapter.json", "r") as f:
...
class T5DenseReluDense(nn.Module):
    def __init__(self):
        ...
        self.wi = nn.Linear(self.d_model, self.d_ff, bias=False)
        self.wo = nn.Linear(self.d_ff, self.d_model, bias=False)
...
class T5LayerFF(nn.Module):
    def __init__(self, config, adapter_config=None):
        ...
        if self.train_adapters:
            if isinstance(adapter_config, metaAda_config) and (adapter_config.unique_hyper_net or adapter_config.efficient_unique_hyper_net):
                self.UniqueHyperNet = True
            else:
                self.UniqueHyperNet = False
                    
            self.train_adapters_blocks = adapter_config.train_adapters_blocks and not self.UniqueHyperNet
                
            if self.train_adapters_blocks:
                self.adapter_controller = HyperLayerAdapter(adapter_config)
                if isinstance(adapter_config, metaAda_config):
                    self.isMetaAdapter = True
                else:
                    self.isMetaAdapter = False
        elif self.UniqueHyperNet:
            self.layer_HyperNet = HyperLayerAdapterUnique(adapter_config)
    ...
    def forward(self, hidden_states, task=None, task_embedding=None, t5_block_adapters=None):
        ...
        if self.train_adapters and self.train_adapters_blocks:
            if not self.isMetaAdapter:
                y = self.adapter_controller(task, y)
            else:
                y = self.adapter_controller(task_embedding, y)
        elif self.UniqueHyperNet and self.train_adapters:
            y = self.layer_HyperNet(y, t5_block_adapters.feed_forward)
        y = self.dropout(y)
...
class T5LayerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.train_adapters and self.train_adapters_blocks:
            ...
            y = self.layer_HyperNet(y, t5_block_adapters.self_attention)
        y = self.dropout(y)
        layer_output = hidden_states + y
...
class T5Stack(nn.Module):
    def __init__(self, config, embed_tokens=None, adapter_config=None):
        ...
        self.train_adapters = config.train_adapters
        if self.train_adapters:
            if isinstance(adapter_config, metaAda_config) and adapter_config.uniques_hyper_net:
                self.UniqueHyperNet = True
            else:
                self.UniqueHyperNet = False
                
            if isinstance(adapter_config, metaAda_config) and adapter_config.uniques_hyper_net:
                self.Eff_UniqueHyperNet = True
            else:
                self.Eff_UniqueHyperNet = False
            
            if self.UniqueHyperNet:
                self.adapter_hyper_net = HyperBlockAdapter(adapter_config, config.num_layers)
                
            if self.Eff_UniqueHyperNet:
                self.adapter_hyper_net = HyperBlockAdapterUnique(adapter_config, config.num_layers)
    ...
    def init_weights(self):
        if self.embed_tokens is not None:
            self.embed_tokens.weight.data.normal_(mean=0.0, std=self.config.initializer_range)
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config, adapter_config=None):
        ...
        # Compute the task embeddings
        self.train_adapters = config.train_adapters
        if config.train_adapters and isinstance(adapter_config, metaAda_config):
            self.task_embedding_controller = TaskEmbedHyperNet(adapter_config)
    ...
    def forward(
        ...
        decoder_outputs = self.decoder(
            ...
            return_dict=return_dict,
            task=task,
            task_embedding=self.task_embedding_controller(task) if (self.train_adapters and isinstance(self.adapter_config, metaAda_config)) else None
        )
...
class T5Test(T5PreTrainedModel):
